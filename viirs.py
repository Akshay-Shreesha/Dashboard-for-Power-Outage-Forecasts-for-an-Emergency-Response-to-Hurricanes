...
def concatenate_preprocessed_vertical(
    north_geotiff_path, south_geotiff_path, output_folder
):
    ...
    Paramaters
    ----------
    north_geotiff_path : str
        Path to the North-most GeoTiff.

    south_geotiff_path : str
        Path to the South-most GeoTiff.
    ...
    print(
        (
            ...
            f"{os.path.basename(north_geotiff_path)}\n    "
            f"{os.path.basename(south_geotiff_path)}"
        )
    )
    ...
    try:
        ...
        concatenated = np.concatenate(
            (
                read_geotiff_into_array(geotiff_path=north_geotiff_path),
                read_geotiff_into_array(geotiff_path=south_geotiff_path),
            ),
            axis=0,
        )
        ...
        export_name = create_concatenated_export_name(
            west_image_path=north_geotiff_path,
            east_image_path=south_geotiff_path,
        )
...
def create_transform_vnp46a3(hdf5):
    """Creates a geographic transform for a VNP46A2 HDF5 file,
    based on longitude bounds, latitude bounds, and cell size.
    ...
    """
    with rio.open(hdf5) as dataset:
        HorizontalTileNumber= int(dataset.tags()["HorizontalTileNumber"])
        VerticalTileNumber = int(dataset.tags()["VerticalTileNumber"])
...
#         latitude_min = int(dataset.tags()["SouthBoundingCoord"])
#         latitude_max = int(dataset.tags()["NorthBoundingCoord"])
        longitude_min = WestBoundCoord
        longitude_max = EastBoundCoord
        latitude_min = SouthBoundCoord
        latitude_max = NorthBoundCoord
        # Extract number of row and columns from first
        #  Science Data Set (subdataset/band)
        with rio.open(dataset.subdatasets[0]) as band:
            num_rows, num_columns = (
...
def extract_band_vnp46a3(hdf5_path, band_name):
    """Extracts the specified band (Science Data Set) from a NASA VNP46A2
    HDF5 file.
...
def preprocess_vnp46a3(hdf5_path, output_folder):
    """Preprocessed a NASA VNP46A2 HDF5 (.h5 file)
    ...
    try:
        print("Extracting bands...")
        # Extract DNB BRDF-Corrected radiance
        dnb_brdf_corrected_ntl = extract_band_vnp46a3(
            hdf5_path=hdf5_path, band_name="AllAngle_Composite_Snow_Free"
        )
...
#         )
        ...
        print("Filling masked values...")
        # Set fill value to np.nan and fill masked values
        ma.set_fill_value(masked_for_fill_value, np.nan)
        filled_data = masked_for_fill_value.filled()

        print("Creating metadata...")
        # Create metadata (for export)
        metadata = create_metadata(
            array=filled_data,
            transform=create_transform_vnp46a3(hdf5_path),
            driver="GTiff",
            nodata=np.nan,
            count=1,
            crs="epsg:4326",
...
def preprocess_vnp46a2_gapfilled(hdf5_path, output_folder):
    """Preprocessed a NASA VNP46A2 HDF5 (.h5 file)
...
def concatenate(input_folder, output_folder, name):
  ...
  for i in tqdm(range(len(udatesid))):
    ...
    for j in range(lenH):
      for k in range(lenV):
        data_read = rasterio.open(files_current[k,j])
        data_arr = data_read.read(1)
        concat_arr[k*s1:(k+1)*s1, j*s2:(j+1)*s2] = data_arr
...
def concatenate_universal(input_folder, output_folder, name):
